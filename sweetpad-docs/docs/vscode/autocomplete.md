...
- **SweetPad's own build server** (the default) reads compiler arguments straight from the project, so
  ...
  `.xcworkspace`.
- **xcode-build-server** is a battle-tested external tool that parses Xcode build logs. Install it separately, and
  build the project once before autocomplete comes alive.

:::info

Swift Packages don't need any of this, because SourceKit-LSP understands `Package.swift` natively. If your workspace root is
...
another tool keeps that tool, so an existing setup is never swapped out underneath you, including a
...
- The server runs through the [`sweetpad` CLI](../cli/getting-started.md), so it needs to be installed:
...
`debug` includes the raw JSON-RPC traffic). With `xcode-build-server`, logging goes to a file instead, so set
`XBS_LOGPATH` via `sweetpad.xcodebuildserver.serverEnv` (see below).

## Auto-regenerate buildServer.json

By default SweetPad regenerates `buildServer.json` whenever you build or change the default scheme, which helps if you
...
You can also pass environment variables to the long-running server process that SourceKit-LSP launches, which helps for
