...
//! compiler-argument resolution ([`build_settings`], [`build_context`]), local
//! SwiftPM package discovery ([`package_members`]), and the Build Server
//! Protocol server ([`bsp`]). It depends on `sweetpad-lib` for the
