...
//!   Build settings fall back to it when the active Xcode can't be located or
//!   parsed.
...
/// The catalog baked into the binary (latest captured Xcode). Build settings
/// use it as the defaults layer when the active Xcode can't be located or
/// parsed.
///
/// The blob stores each `sdk_paths` entry relative to the captured Xcode's
/// developer dir (see [`serialize_embedded`]); they come back joined onto the
/// catalog's own `developer_dir`, the same root `DEVELOPER_DIR` and
/// `PLATFORM_DIR` resolve under.
pub fn embedded() -> Result<Catalog, Error> {
    let (mut catalog, _) = deserialize(EMBEDDED)?;
    if let Some(developer_dir) = catalog.developer_dir.as_deref().map(PathBuf::from) {
        for path in catalog.sdk_paths.values_mut() {
            *path = developer_dir.join(&*path);
        }
    }
    Ok(catalog)
}

/// Serialize a catalog parsed from a captured `xcspec-cache/xcode-<ver>` tree
/// as the blob [`embedded`] reads. `sdk_root` is that tree's `sdksettings`
/// directory, which mirrors the captured Xcode's developer dir: every
/// `sdk_paths` entry under it is stored relative to it, so the blob names
/// nothing on the machine or checkout that generated it.
#[must_use]
pub fn serialize_embedded(catalog: &Catalog, sdk_root: &Path) -> Vec<u8> {
    let mut portable = catalog.clone();
    for path in portable.sdk_paths.values_mut() {
        if let Ok(relative) = path.strip_prefix(sdk_root) {
            *path = relative.to_path_buf();
        }
    }
    serialize(&portable, 0)
}
...
fn write_catalog(out: &mut Vec<u8>, c: &Catalog) {
    ...
    write_opt_str(out, c.product_build_version.as_deref());
...
fn read_catalog(r: &mut Reader) -> Result<Catalog, Error> {
    Ok(Catalog {
        ...
        product_build_version: r.opt_str()?,
...
mod tests {
    ...
    fn sample() -> Catalog {
        let mut c = Catalog {
            ...
            product_build_version: Some("17F42".into()),
    ...
    fn assert_same(a: &Catalog, b: &Catalog) {
        ...
        assert_eq!(a.product_build_version, b.product_build_version);
    ...
    #[test]
    fn embedded_sdk_paths_resolve_under_its_developer_dir() {
        let catalog = embedded().expect("the embedded blob deserializes");
        let developer_dir = PathBuf::from(catalog.developer_dir.as_deref().unwrap());
        assert_eq!(
            catalog.sdk_paths.get("macosx").and_then(|p| p.parent()),
            Some(&*developer_dir.join("Platforms/MacOSX.platform/Developer/SDKs"))
        );
        for (name, path) in &catalog.sdk_paths {
            assert!(
                path.starts_with(&developer_dir),
                "{name}: {}",
                path.display()
            );
        }
    ...
    /// checkout lives.
    #[test]
    fn the_embedded_blob_regenerates_byte_for_byte() {
        let (catalog, _) = deserialize(EMBEDDED).expect("the embedded blob deserializes");
        let version = catalog.xcode_version.expect("the blob names its Xcode");
        let root =
            Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("xcspec-cache/xcode-{version}"));
        let sdk_root = root.join("sdksettings");
        let fresh = xcspec::load_catalog(&root, Some(&sdk_root)).expect("the capture parses");
        assert!(
            serialize_embedded(&fresh, &sdk_root) == EMBEDDED,
            "src/catalog_embedded.bin is stale: run `cargo run --release --example gen_embedded_catalog`"
        );
    }
}
