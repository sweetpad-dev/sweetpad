...
//! Xcode's `project.pbxproj` ([`crate::pbxproj_merge`]) and SwiftPM's
//! `Package.resolved` ([`crate::spm_resolved`]). Both are driven through the
...
pub fn merge_text(
    ...
    match kind {
        Kind::Pbxproj => {
            ...
            let merged = crate::pbxproj_merge::merge(b.as_ref(), &o, &t);
            let text = crate::pbxproj_writer::serialize(&merged.value, &project_name_for(pathname));
            Ok(MergeText {
                text,
        ...
        Kind::Spm => {
            ...
            let o = parse("ours", ours)?;
            let t = parse("theirs", theirs)?;
            let merged = crate::spm_resolved::merge(b.as_ref(), &o, &t);
            let text = crate::spm_resolved::serialize(&merged.value);
            Ok(MergeText {
                text,
                conflicts: merged.conflicts,
            })
