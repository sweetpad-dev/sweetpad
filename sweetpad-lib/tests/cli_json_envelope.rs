...
fn parse_stderr_error(out: &Output, args: &[&str]) -> Value {
    ...
    let v: Value = serde_json::from_str(stderr.trim())
        .unwrap_or_else(|e| panic!("{args:?}: stderr is not a JSON error envelope ({e}):\n{stderr:?}"));
...
fn success_payloads_are_enveloped() {
    ...
    let commands: &[&[&str]] = &[
        &["context", "show", "--project", proj, "--json", "--non-interactive"],
        &["derived-data", "path", "--project", proj, "--json", "--non-interactive"],
        &["derived-data", "size", "--project", proj, "--json", "--non-interactive"],
    ];
    for args in commands {
        ...
        assert!(v.get("data").is_some(), "{args:?}: success envelope needs data");
    }
}
