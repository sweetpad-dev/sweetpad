...
import { ENV_PREVIEW_APPEARANCE, ENV_PREVIEW_ID, PREVIEW_HOST_BOOTSTRAP } from "./host-bootstrap.js";
 ...
 *   (using a vendored `__swift5_proto` discovery — no external dependency).
...
export class PreviewHostManager {
  ...
  async scaffold(): Promise<vscode.Uri> {
    ...
    void vscode.window.showInformationMessage(
      "SweetPad: Added SweetPadPreviewHost.swift (no dependencies). Call SweetPadPreviewHost.rootView() from your @main App — see the file header. Then Build & Run once on a simulator.",
    );
    return uri;
  }
