...
import { refreshShellEnv } from "../../core/tasks/shell-env";
import { type AppDeps, resetSweetPadState } from "../commands";
...
export async function refreshShellEnvCommand(_deps: AppDeps) {
  await refreshShellEnv({
    logger: commonLogger,
    shell: workspaceConfig.get("shellEnv.shell"),
    timeoutMs: workspaceConfig.get("shellEnv.timeout"),
  });
}
