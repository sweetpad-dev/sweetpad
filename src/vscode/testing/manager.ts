...
import type { UserAsker } from "../../core/asker/types.js";
import type { BuildManager } from "../../core/build/manager.js";
import {
  askXcodeWorkspacePath,
  detectWorkspaceType,
  getSwiftPMDirectory,
  getXcodeBuildDestinationString,
} from "../../core/build/utils.js";
import type { XcodeCliDeps } from "../../core/cli/scripts.js";
import { getBuildSettingsToAskDestination, getXcodeBuildCommand } from "../../core/cli/scripts.js";
import type { ConfigProvider } from "../../core/config/types.js";
import type { DestinationsManager } from "../../core/destination/manager.js";
import type { Destination } from "../../core/destination/types.js";
import type { ExecutionScopeService } from "../../core/execution-scope.js";
import { exec } from "../../core/exec.js";
import { isFileExists } from "../../core/files.js";
import type { Logger } from "../../core/logger/types.js";
import type { TaskRunner } from "../../core/tasks/types.js";
import { assertUnreachable } from "../../core/types.js";
import type { WorkspaceRoot } from "../../core/workspace-root.js";
import type { VsCodeWorkspaceState } from "../adapters/state.js";
...
export class TestingManager {
  ...
  private workspace: VsCodeWorkspaceState;
  ...
  private makeXcodeCliDeps(): XcodeCliDeps {
    return { cwd: this.workspacePath, config: this.config, logger: this.logger };
  }

  private makeAskBuildDeps() {
    return {
      ...this.makeXcodeCliDeps(),
      asker: this.asker,
  ...
  async askTestingConfigurations(): Promise<{
    ...
    const xcworkspace = await askXcodeWorkspacePath(this.makeAskBuildDeps(), this.buildManager);
    const scheme = await askSchemeForTesting({
      progress: this.progress,
      buildManager: this.buildManager,
      cliDeps: this.makeXcodeCliDeps(),
      xcworkspace: xcworkspace,
      title: "Select a scheme to run tests",
    });
    const configuration = await askConfigurationForTesting({
      asker: this.asker,
      buildManager: this.buildManager,
      config: this.config,
      cliDeps: this.makeXcodeCliDeps(),
    ...
    const buildSettings = await getBuildSettingsToAskDestination(this.makeXcodeCliDeps(), {
      scheme: scheme,
    ...
    await this.taskRunner.run({
      ...
      callback: async (terminal) => {
        ...
        await terminal.execute({
          command: getXcodeBuildCommand(this.makeXcodeCliDeps()),
          args: args,
          cwd: cwd,
          closeStdin: true,
        });
  ...
  async resolveSPMTestingTarget(options: { queue: vscode.TestItem[]; xcworkspace: string }) {
    ...
    const workscePath = this.workspaceRoot.getPath();
    ...
    for (const test of queue) {
      ...
      for (const ancestorPath of getAncestorsPaths({
        ...
        try {
          ...
          this.logger.error("Failed to get test target name", {
            error: error,
          });
  ...
  async runTests(options: {
    ...
    for (const test of queue) {
      this.logger.debug("Running single test from queue", {
        testId: test.id,
      ...
      const defaultTarget = await askTestingTarget({
        testingManager: this,
        cliDeps: this.makeXcodeCliDeps(),
        xcworkspace: xcworkspace,
