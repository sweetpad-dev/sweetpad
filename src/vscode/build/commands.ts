...
import { type XcodeScheme, getBuildConfigurations, getIsXcodeBuildServerInstalled } from "../../core/cli/scripts";
...
export async function resolveDependenciesCommand(deps: AppDeps, item?: BuildTreeItem) {
  ...
  const scheme =
    item?.scheme ??
    (await askSchemeForBuild(makeAskBuildDeps(deps), deps.buildManager, {
...
export async function generateBuildServerConfigCommand(deps: AppDeps, item?: BuildTreeItem) {
  ...
  const isServerInstalled = await getIsXcodeBuildServerInstalled(makeXcodeCliDeps(deps));
  ...
  await refreshBuildServer(
    { ...makeXcodeCliDeps(deps), lsp: deps.lspRefresher },
    {
      xcworkspace: xcworkspace,
      scheme: scheme,
  ...
  vscode.window.showInformationMessage("buildServer.json generated in workspace root", "Open").then((selected) => {
    if (selected === "Open") {
      const workspacePath = deps.workspaceRoot.getPath();
...
export async function selectXcodeWorkspaceCommand(deps: AppDeps) {
  deps.progressStatusBar.updateText("Searching for workspace");
  const workspace = await selectXcodeWorkspace(makeAskBuildDeps(deps), {
    autoselect: false,
  });
  const updateAnswer = await showYesNoQuestion(deps.asker, {
    title: "Do you want to update path to xcode workspace in the workspace settings (.vscode/settings.json)?",
  });
  if (updateAnswer) {
    const relative = deps.workspaceRoot.getRelativePath(workspace);
...
export async function diagnoseBuildSetupCommand(deps: AppDeps): Promise<void> {
  ...
  await deps.taskRunner.run({
    ...
    callback: async (terminal) => {
      ...
      const xcWorkspacePath = getCurrentXcodeWorkspacePath({
        config: deps.config,
      ...
      try {
        ...
        if (e instanceof ExecBaseError) {
          ...
          if (strerr?.includes("does not contain an Xcode project, workspace or package")) {
            ...
            const xcodepaths = await detectXcodeWorkspacesPaths(deps.workspaceRoot.getPath());
...
export async function refreshSchemesCommand(deps: AppDeps): Promise<void> {
  ...
  if (!xcworkspace) {
    ...
    await askXcodeWorkspacePath(makeAskBuildDeps(deps), deps.buildManager);
    return;
  }
...
export async function switchWorktreeCommand(deps: AppDeps) {
  ...
  const worktrees = await detectGitWorktrees({ cwd: deps.workspaceRoot.getPath(), logger: deps.logger });
  if (worktrees.length <= 1) {
    vscode.window.showInformationMessage("No additional git worktrees found. Create one with `git worktree add`.");
    return;
  }

  const currentWorkspace = getCurrentXcodeWorkspacePath({
    config: deps.config,
    state: deps.workspace,
    cwd: deps.workspaceRoot.getPath(),
  });
  ...
  const relative = deps.workspaceRoot.getRelativePath(selected.context.xcworkspace);
  await updateWorkspaceConfig("build.xcodeWorkspacePath", relative);
