...
export class SchemeWatcher implements vscode.Disposable {
  ...
  async start(): Promise<void> {
    ...
    const isEnabled = this.config.get("build.autoRefreshSchemes") ?? true;
    if (!isEnabled) {
      this.logger.log("Scheme auto-refresh is disabled", {
    ...
    this.logger.log("Scheme watcher started", {
      workspacePath: this.workspacePath,
    });
  }
  ...
  private async setupWatchers() {
    ...
    const isProjectYmlExists = await isFileExists(path.join(this.workspacePath, "project.yml"));
    ...
    const isTuistProjectExists = await isFileExists(path.join(this.workspacePath, "Project.swift"));
    const isTuistWorkspaceExists = await isFileExists(path.join(this.workspacePath, "Workspace.swift"));
  ...
  handleChange(e: vscode.Uri, reason: string) {
    this.logger.log("Scheme watcher detected changes", {
      workspacePath: this.workspacePath,
      file: e.fsPath,
      reason: reason,
    });
    ...
    if (this.derivedDataPath && e.fsPath.startsWith(this.derivedDataPath)) {
      this.logger.debug("Skipping file in derived data path", {
        file: e.fsPath,
    ...
    if (buildPaths.some((buildPath) => e.fsPath.includes(buildPath))) {
      this.logger.debug("Skipping file in build output directory", {
        file: e.fsPath,
      });
      return;
    }
    ...
    const refreshDelay = this.config.get("build.autoRefreshSchemesDelay") ?? 500;
    ...
    this.throttle = setTimeout(() => {
      ...
      this.buildManager
        .refreshSchemes()
        .then(() => {
          this.logger.log("Schemes auto-refreshed successfully", {
            workspacePath: this.workspacePath,
            trigger: reason,
        ...
        .catch((error) => {
          this.logger.error("Failed to auto-refresh schemes", {
            workspacePath: this.workspacePath,
            trigger: reason,
            file: e.fsPath,
            error: error,
  ...
  dispose() {
    ...
    this.logger.log("Scheme watcher stopped", {
      workspacePath: this.workspacePath,
    });
  }
}
