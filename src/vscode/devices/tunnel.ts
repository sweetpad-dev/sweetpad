...
export class TunnelManager implements vscode.Disposable {
  ...
  async autoConnect(): Promise<void> {
    if (!this.config.get("build.deviceTunnelAutoStart")) return;
    await this.connect();
  }
  ...
  private async connectInner(): Promise<void> {
    ...
    const pymd3 = this.config.get("build.pymobiledevice3Path") ?? "pymobiledevice3";
    ...
    if (!(await this.isPymobiledevice3Available(pymd3))) {
      return;
    }
  ...
  private async isPymobiledevice3Available(binaryPath: string): Promise<boolean> {
    try {
      await exec({
        command: binaryPath,
        args: ["version"],
