...
import { getXcodeVersionInstalled } from "../../core/cli/scripts.js";
  ...
  implements vscode.DocumentFormattingEditProvider, vscode.DocumentRangeFormattingEditProvider
{
  ...
  async getFormatterCommand(
    ...
    const rawArgs = this.config.get("format.args");
    ...
    const path = this.config.get("format.path");
    if (path) {
  ...
  private getCustomRangeArgument(document: vscode.TextDocument, ranges: vscode.Range[]): string[] {
    const selectionArgs = this.config.get("format.selectionArgs");
    if (!selectionArgs) {
      return [];
    }
