 ...
 * a single stable launcher path — a generated shim in `.sweetpad/` — so the
 * config carries neither the volatile VS Code Electron path nor any env. The
 * shim sets `ELECTRON_RUN_AS_NODE` (plus the log path / user serverEnv) and execs
 * the bundled `out/bsp-server.js` through VS Code's Node, which loads the addon.
 *
 * Project/Xcode/derived-data aren't baked in either: the server discovers the
 * control socket at runtime and pulls them via `bsp.resolveConfig`.
 */
async function generateSweetpadBuildServerConfig(): Promise<void> {
  ...
  // The launcher env. `ELECTRON_RUN_AS_NODE` makes VS Code's binary behave as
  // Node; the debug log path and any user serverEnv ride along.
  ...
  // The shim keeps buildServer.json's `argv` a single stable path instead of
  // `/usr/bin/env KEY=VAL … <Electron helper> <launcher>` — and absorbs the
  // volatile Electron path, which changes across VS Code updates.
  const shimDir = getStateRoot(cwd);
  await fs.mkdir(shimDir, { recursive: true });
  const shimPath = path.join(shimDir, "bsp-server");
  const exports = Object.entries(prepareEnvVars(env))
    .filter(([, value]) => value !== undefined)
    .map(([key, value]) => `export ${key}=${quote([value as string])}`);
  const shim = [
    "#!/bin/sh",
    "# Generated by SweetPad — launches the bundled BSP server through VS Code's",
    "# Node runtime. Safe to delete; re-run 'SweetPad: Generate Build Server Config' to restore.",
    ...exports,
    `exec ${quote([process.execPath, bspServer])} "$@"`,
    "",
  ].join("\n");
