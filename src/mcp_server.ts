...
import { takeScreenshotSchema, takeScreenshotImplementation, TakeScreenshotArgs } from "./tools/screenshotTool";
...
export function createMcpServer(options: McpServerOptions, extensionContext: ExtensionContext): McpServerInstance {
  ...
  const buildResolveDependencies = createCommandTool(
  ...
  server.tool(
    buildResolveDependencies.toolName,
    "Resolve Swift Package Manager dependencies",
    buildResolveDependencies.schema,
    buildResolveDependencies.implementation,
  );

  const buildSelectXcodeWorkspace = createCommandTool(
    "sweetpad.build.selectXcodeWorkspace",
    "select_xcode_workspace",
    "Select Xcode workspace file",
    extensionContext,
  );
  server.tool(
    buildSelectXcodeWorkspace.toolName,
    "Select Xcode workspace file",
    buildSelectXcodeWorkspace.schema,
    buildSelectXcodeWorkspace.implementation,
  );
