...
export class BazelParser {
   ...
   */
  private static parseTargets(content: string, filePath?: string): { targets: BazelTarget[], testTargets: BazelTarget[] } {
  ...
  private static parseTopLevelTargets(
    ...
    while ((targetMatch = topLevelRegex.exec(content)) !== null) {
      ...
      const isTest = targetName.toLowerCase().includes('test') || 
                    targetLabel.toLowerCase().includes('/tests');
