...
export class WorkspaceGroupTreeItem extends vscode.TreeItem {
  ...
  constructor(options: {
    ...
    if (path.basename(options.workspacePath) === "Package.swift") {
      ...
      path.basename(options.workspacePath) === "BUILD.bazel" ||
      path.basename(options.workspacePath) === "BUILD"
    ) {
    ...
    // Use xcworkspace icon for .xcworkspace files
      ...
      this.workspacePath.endsWith(".xcworkspace") ||
      this.workspacePath.includes(".xcworkspace/") ||
      this.workspacePath.includes("project.xcworkspace")
    ) {
      this.iconPath = vscode.Uri.joinPath(
        vscode.Uri.file(this.provider.context?.extensionPath || ""),
        "images",
