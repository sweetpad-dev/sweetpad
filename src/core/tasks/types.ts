...
/**
 * Wraps a unit of work (build, run, test, clean) in a managed terminal/task
 * pair. Implementations decide how to surface progress (VS Code task UI vs
 * plain stdout) and how to enforce per-lock mutual exclusion.
 */
export interface TaskRunner {
  run<TMetadata>(options: {
    name: string;
    source?: string;
    error?: string;
  ...
  /**
   * Terminate any running task whose `lock` and `metadata` match the predicate.
   * Used to stop in-flight builds by scheme.
   */
