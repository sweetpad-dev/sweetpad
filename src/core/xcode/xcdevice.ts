 ...
 */
export async function listDevicesWithXcdevice(options: { cwd: string; logger: Logger }): Promise<XcdeviceDevice[]> {
  ...
  try {
    ...
    logger.debug("xcdevice list output", { stdout });
    ...
    logger.error("Failed to list devices with xcdevice", { error });
    return [];
  }
}
