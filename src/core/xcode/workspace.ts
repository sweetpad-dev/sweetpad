...
export class XcodeWorkspace {
  ...
  constructor(options: { xcworkspacePath: string; children: XcodeWorksaceItem[]; logger: Logger }) {
  ...
  async parseProject(projectPath: string): Promise<XcodeProject | null> {
    try {
      ...
      this.logger.error("Failed to parse xcode project", {
        error: error,
  ...
  async getGroupChildrenProjects(options: {
    ...
    for (const child of children) {
      if (child.type === "group") {
        ...
        this.logger.debug("Resolved project path", { projectPath: projectPath });
        if (!projectPath) continue;

        if (projects.some((proj) => proj.projectPath === projectPath)) {
          this.logger.debug("Project is duplicated", { projectPath: projectPath });
          continue;
        }
  ...
  buildLocation(item: XcodeWorksaceItem, ancestors: XcodeWorkspaceGroup[]): string | null {
    ...
    if (location.obj === "developer") {
      this.logger.debug("Location is developer", { location: location });
      return null;
    }
    ...
    if (location.obj === "absolute") {
      this.logger.debug("Location is absolute (2)", { location: location });
      if (path.isAbsolute(location.path)) {
        return location.path;
      }
      return path.join(this.xcworkspaceParentPath, location.path);
    }
    this.logger.debug("Unknown location type", { location: location });
  ...
  async resolveProjectPath(ancestors: XcodeWorkspaceGroup[], fileRef: XcodeWorkspaceFileRef): Promise<string | null> {
    ...
    if (xcworkspaceParent.endsWith(projectExtension)) {
      this.logger.debug("Parent of xcworkspace is xcodeproj", { xcworkspaceParent: xcworkspaceParent });
    ...
    if (path.isAbsolute(filRefPath)) {
      this.logger.debug("FileRef path is absolute", { filRefPath: filRefPath });
    ...
    if (!resolvedPath) {
      this.logger.debug("No resolved path", { fileRef: fileRef });
      return null;
    }

    this.logger.debug("Resolved path", { resolvedPath: resolvedPath });
   ...
   */
  static async parseWorkspace(xcworkspacePath: string, options: { logger: Logger }): Promise<XcodeWorkspace> {
    ...
    return XcodeWorkspace.parseContentsWorkspaceData(contentsPath, options);
  }
   ...
   */
  static async parseContentsWorkspaceData(contentsPath: string, options: { logger: Logger }): Promise<XcodeWorkspace> {
