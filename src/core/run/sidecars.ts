...
import type { ProcessGroup, ProcessSpec, TaskTerminal } from "../tasks/types";
import {
  ANSI_ESCAPE_RE,
  LEVEL_COLOR,
  LEVEL_LETTER,
...
abstract class LogSidecar {
  ...
  isLogStreamEnabled(): boolean {
    return this.config.get("build.logStreamEnabled") ?? true;
  }
  ...
  buildPredicate(bundleId: string, executableName: string): string {
    const custom = this.config.get("build.logStreamPredicate");
    if (custom) {
...
export class Pymd3Sidecar extends LogSidecar {
  ...
  constructor(
    ...
    super(group, deps);
    this.rawExtraArgs = this.config.get("build.pymobiledevice3ExtraArgs") ?? [];
    ...
    this.filter = filterExecutable
      ? this.buildPymd3Filter({
          ...
          subsystemDenyList: this.config.get("build.pymobiledevice3SubsystemDenyList") ?? ["com.apple.*"],
          subsystemAllowList: this.config.get("build.pymobiledevice3SubsystemAllowList") ?? [],
        })
      : null;
  }
  ...
  async spec(): Promise<ProcessSpec | null> {
    ...
    const binaryPath = this.config.get("build.pymobiledevice3Path") ?? "pymobiledevice3";
    ...
    this.logger.debug("pymobiledevice3 log stream", {
  ...
  async isPymobiledevice3Available(binaryPath: string): Promise<boolean> {
    try {
      await exec({ command: binaryPath, args: ["version"], cwd: this.cwd, logger: this.logger });
      return true;
    } catch {
      return false;
    }
