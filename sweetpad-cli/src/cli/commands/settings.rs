//! `sweetpad settings …` — show, set, and unset build settings.
//!
//! `show` displays *resolved* settings, computed in-process by the resolver
//! ([`sweetpad_core::build_settings`]) — the engine's specialty; `show --raw`
//! prints the stored pbxproj layer instead (the one `set`/`unset` edit), which
//! is why a value can survive an `unset` in the resolved view: it's inherited
//! from another layer.
//!
//! `set`/`unset` mutate `XCBuildConfiguration.buildSettings` via
//! [`sweetpad_lib::settings_pbxproj`] (CLI_DESIGN §9f). Mutations never guess:
//! an ambiguous workspace, unknown target, or unknown configuration is a hard
...
use crate::cli::{
    BuildTargetArgs, CliError, CommandResult, ContainerArgs, Context, Render, Rendered,
};
...
pub enum Action {
    ...
    Show {
        ...
        /// Show the raw pbxproj layer (what `set`/`unset` edit) instead of
        /// resolved values.
        #[arg(long)]
        raw: bool,
    },
    /// Set build settings in the project file.
    Set(SetArgs),
    /// Remove build settings from the project file.
    Unset(UnsetArgs),
}

/// Flags for `settings set`.
#[derive(Debug, Args)]
pub struct SetArgs {
...
pub fn run(ctx: &mut Context, action: &Action) -> CommandResult {
    match action {
        Action::Show {
            ...
            if *raw {
                show_raw(ctx, target.as_deref(), key.as_deref())
            } else {
...
/// The `settings set`/`unset` report: per-configuration before → after, the
/// recorded Info.plist exceptions, xcspec/xcconfig warnings, and the
/// re-resolved values (JSON only — human mode shows the raw edit).
...
/// One scope's stored settings for `show --raw`: the project, or one target.
struct RawScope {
    target: Option<String>,
    configurations: Vec<settings_pbxproj::ConfigSettings>,
}

/// The `settings show --raw` payload: the pbxproj layer per scope, optionally
/// filtered to one `--key` (bare values in human mode, like resolved `--key`).
struct RawResult {
    scopes: Vec<RawScope>,
...
fn show_raw(ctx: &mut Context, target: Option<&str>, key: Option<&str>) -> CommandResult {
    let container = resolve::container(ctx)?;
    let targets: Vec<String> = target.map(str::to_string).into_iter().collect();
