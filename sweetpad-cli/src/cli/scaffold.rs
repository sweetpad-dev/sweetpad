...
//! (assembled as a
//! [`sweetpad_lib::pbxproj`] object graph and serialized by [`sweetpad_lib::pbxproj_writer`]),
...
use sweetpad_lib::pbxproj::{Dict, Value};
use sweetpad_lib::xcscheme::Element;
...
pub fn scaffold(spec: &ProjectSpec) -> Vec<ScaffoldFile> {
    ...
    let pbxproj = sweetpad_lib::pbxproj_writer::serialize(&project_graph(spec), name);
    let scheme = sweetpad_lib::xcscheme::serialize(&scheme_element(spec));
...
mod tests {
    ...
    fn pbxproj_parses_and_round_trips() {
        ...
        let value = sweetpad_lib::pbxproj::parse(raw).expect("generated pbxproj parses");

        // … and re-serialize to exactly the same bytes (self-consistent).
        let again = sweetpad_lib::pbxproj_writer::serialize(&value, "MyApp");
    ...
    fn project_resolves_target_and_configurations() {
        let files = scaffold(&spec());
        let value = sweetpad_lib::pbxproj::parse(pbxproj(&files)).unwrap();
        let project =
            sweetpad_lib::project::open_from_value(&value, std::path::Path::new("MyApp.xcodeproj"))
    ...
    fn scheme_is_well_formed_xml() {
        let xml = sweetpad_lib::xcscheme::serialize(&scheme_element(&spec()));
        // Parse it back to confirm the generated scheme is valid.
        let root = sweetpad_lib::xcscheme::parse(&xml).expect("scheme parses");
    ...
    fn macos_uses_macos_sdk_and_omits_ios_keys() {
        ...
        let value = sweetpad_lib::pbxproj::parse(raw).expect("macOS pbxproj parses");
        let project = sweetpad_lib::project::open_from_value(
            &value,
            std::path::Path::new("MacApp.xcodeproj"),
        )
        .expect("resolves");
