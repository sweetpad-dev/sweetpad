...
export class TuistGenWatcher implements vscode.Disposable {
  ...
  async start(): Promise<void> {
    ...
    // was built. Rebuild it whenever the window's folders change: otherwise manifests added after
    // activation are never watched, and a folder that leaves keeps a watcher that can still fire
    // a generate in a directory no longer in the window.
    this.foldersSubscription = vscode.workspace.onDidChangeWorkspaceFolders(() => {
      void this.rebuild().catch((error) => {
        commonLogger.error("Failed to rebuild tuist watchers", { error: error });
      });
    });
    await this.rebuild();
  }
  ...
  private async applyWatchers(): Promise<void> {
    ...
    const folders = isEnabled ? await workspaceFoldersContaining("Project.swift", "Workspace.swift") : [];
    if (this.disposed) return;

    this.disposeWatchers();
    if (folders.length === 0) {
      commonLogger.log("No Project.swift or Workspace.swift in any workspace folder, tuist watcher idle");
      return;
    }
